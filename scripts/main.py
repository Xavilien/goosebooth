...
def redrawAll(app):
    ...
    for i, (cx, cy) in enumerate(app.gooseSteps):
        trees.drawTernateShape(app, cx - 10, cy + 10, h=app.gooseStepSize, fill='saddleBrown', opacity=20 + i * 20,
                               dir=app.gooseStepsDirs[i])
        trees.drawTernateShape(app, cx + 10, cy - 10, h=app.gooseStepSize, fill='saddleBrown', opacity=20 + i * 20,
                               dir=app.gooseStepsDirs[i])
...
def onKeyPress(app, key):
    ...
    if key == 'e':
        ...
        for house in app.houses[3:]:
            if goose.isNear(app, house):
                app.gooseSize += 20
                goose.getGooseSprite(app, app.gooseDirection)
...
def onStep(app):
    ...
    if app.gooseIsWalk:
        ...
        if app.steps % 5 == 0:
            app.gooseSteps.append(
                (app.width / 2 + random.randrange(-8, 8), app.height / 2 + 30 + random.randrange(-8, 8)))
